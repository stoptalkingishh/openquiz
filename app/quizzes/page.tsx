                    ...
                    <div className="space-y-4">
                        ...
                        {aiPlan && <section className="rounded-xl border-2 border-primary/30 bg-primary/5 p-4 space-y-3">
                            ...
                            <div className="space-y-3">
                                {aiPlan.quizzes.map((planned, index) => <div key={index} className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-3 space-y-3">
                                    <label className="flex items-start gap-2 text-sm font-semibold"><input type="checkbox" className="mt-1" checked={planned.selected} onChange={e => setAiPlan(plan => plan ? { ...plan, quizzes: plan.quizzes.map((item, i) => i === index ? { ...item, selected: e.target.checked } : item) } : plan)} /><span>Generate this quiz</span></label>
                                    <label className="block text-xs font-bold text-neutral-600 dark:text-neutral-400">Quiz title<input value={planned.title} onChange={e => setAiPlan(plan => plan ? { ...plan, quizzes: plan.quizzes.map((item, i) => i === index ? { ...item, title: e.target.value } : item) } : plan)} className="input-field text-sm mt-1" /></label>
                                    <label className="block text-xs font-bold text-neutral-600 dark:text-neutral-400">Description <span className="font-normal">(shown on the quiz card)</span><textarea value={planned.description} onChange={e => setAiPlan(plan => plan ? { ...plan, quizzes: plan.quizzes.map((item, i) => i === index ? { ...item, description: e.target.value } : item) } : plan)} className="input-field text-sm min-h-16 mt-1" /></label>
                                    <div className="grid grid-cols-2 gap-2"><label className="block text-xs font-bold text-neutral-600 dark:text-neutral-400">Tags <span className="font-normal">(comma-separated)</span><input value={planned.tags.join(', ')} onChange={e => setAiPlan(plan => plan ? { ...plan, quizzes: plan.quizzes.map((item, i) => i === index ? { ...item, tags: e.target.value.split(',').map(tag => tag.trim()).filter(Boolean) } : item) } : plan)} className="input-field text-sm mt-1" placeholder="security, SIEM" /></label><label className="block text-xs font-bold text-neutral-600 dark:text-neutral-400">Questions to generate <span className="font-normal">(8–25)</span><input type="number" min="8" max="25" value={planned.questionCount} onChange={e => setAiPlan(plan => plan ? { ...plan, quizzes: plan.quizzes.map((item, i) => i === index ? { ...item, questionCount: Math.min(25, Math.max(8, Number(e.target.value) || 8)) } : item) } : plan)} className="input-field text-sm mt-1" /></label></div>
                                    <label className="block text-xs font-bold text-neutral-600 dark:text-neutral-400">Generation instructions <span className="font-normal">(optional: question style, difficulty, emphasis)</span><textarea value={planned.instructions} onChange={e => setAiPlan(plan => plan ? { ...plan, quizzes: plan.quizzes.map((item, i) => i === index ? { ...item, instructions: e.target.value } : item) } : plan)} className="input-field text-sm min-h-16 mt-1" placeholder="For example: use CySA+ scenario questions and explain why each answer is correct." /></label>
                                    <p className="text-xs text-neutral-500 dark:text-neutral-400"><strong>AI-selected scope:</strong> {planned.scope}</p>
                                </div>)}
                            </div>
                        </section>}
